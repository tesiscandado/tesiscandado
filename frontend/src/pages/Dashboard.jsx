    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          <div key={label} className={`bg-gray-900 border rounded-xl p-4 text-center ${alerta ? 'border-red-700' : 'border-gray-800'}`}>
            <p className={`text-3xl font-bold ${alerta ? 'text-red-400' : 'text-white'}`}>{valor}</p>
      ...
      {/* Estado candados */}
      <section>
        <h3 className="text-gray-400 text-xs uppercase tracking-wider mb-3">Estado de candados</h3>
          ...
          {data.candados.map(c => (
            <div key={c.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-medium truncate">{c.descripcion || c.codigo_dispositivo}</p>
                <p className="text-gray-500 text-xs capitalize">{c.estado}</p>
              </div>
      ...
      <section>
        ...
        {data.alarmas_activas.length === 0 ? (
          <p className="text-gray-500 text-sm">Sin alarmas activas</p>
        ) : (
            ...
            {data.alarmas_activas.map(a => (
              <div key={a.id} className="bg-red-950 border border-red-800 rounded-xl p-4 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-400 text-xs">{a.eventos?.candados?.descripcion} — {new Date(a.eventos?.ocurrido_en).toLocaleString()}</p>
                </div>
      ...
      {/* Eventos recientes */}
      <section>
        <h3 className="text-gray-400 text-xs uppercase tracking-wider mb-3">Últimos eventos</h3>
            ...
            <tbody>
              {data.eventos_recientes.map(e => (
                <tr key={e.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/40">
                  ...
                  <td className="px-4 py-3 text-gray-500 whitespace-nowrap">{new Date(e.ocurrido_en).toLocaleString()}</td>
                </tr>
              ))}
              {data.eventos_recientes.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-500">Sin eventos aún</td></tr>
              )}
            </tbody>
          </table>
        </div>
